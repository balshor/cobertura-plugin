...
public class CoberturaCoverageParser {
  ...
  public static CoverageResult parse(InputStream in, CoverageResult cumulative, Set<String> sourcePaths)
      throws IOException {
    if (in == null)
...
class CoberturaXmlHandler extends DefaultHandler {
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
    ...
    updateMetrics(attributes);
  }

  private void updateMetrics(Attributes attributes) {
    final String linesCovered = attributes.getValue("lines-covered");
    final String linesValid = attributes.getValue("lines-valid");
    if (linesCovered != null && linesValid != null) {
      try {
        rootCoverage.updateMetric(CoverageMetric.LINE,
            Ratio.create(Integer.parseInt(linesCovered), Integer.parseInt(linesValid)));
      } catch (NumberFormatException e) {
        // ignore
      }
    }

    final String branchesCovered = attributes.getValue("branches-covered");
    final String branchesValid = attributes.getValue("branches-valid");
    if (branchesCovered != null && linesValid != null) {
      try {
        rootCoverage.updateMetric(CoverageMetric.CONDITIONAL,
            Ratio.create(Integer.parseInt(branchesCovered), Integer.parseInt(branchesValid)));
      } catch (NumberFormatException e) {
        // ignore
      }
    }
